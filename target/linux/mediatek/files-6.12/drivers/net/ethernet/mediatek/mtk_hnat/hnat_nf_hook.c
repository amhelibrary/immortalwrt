			    ...
			    struct flow_offload_hw_path *hw_path)
{
	...
	/* Flowtable mode: use a mainline-style flow_cls_offload template
	 * if one was prepared for this PPE/hash.  The template already has
	 * the complete orig/new tuple and egress MAC; hnat_entry_bind below
	 * still fills the current output port / DSA tag / QoS path.
	 */
	if (hnat_flow_offload_get_template(skb_hnat_ppe(skb),
					   skb_hnat_entry(skb), foe, &entry)) {
		if (IS_IPV4_GRP(&entry))
			h_proto = ETH_P_IP;
		else if (IS_IPV6_GRP(&entry))
			h_proto = ETH_P_IPV6;
