...
static inline void __RtmpOSTaskCustomize(OS_TASK *pTask)
{
	...
	daemonize();
	reparent_to_init();
	strncpy(current->comm, &pTask->taskName[0], sizeof(pTask->taskName));
	...
	void (*setup)(struct net_device *))
{
	...
	setup(dev);
	strncpy(dev->name, mask, sizeof(dev->name));
	return dev;
}
...
int csi_genl_recv_doit(struct sk_buff *skb_temp, struct genl_info *info)
{
	...
	if (strlen(cmd_msg)) {
		...
		strncpy(dev_string, temp_str, sizeof(dev_string));
		...
		strncpy(req_type, temp_str, sizeof(req_type));
	} else {
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_CSI, DBG_LVL_ERROR,
