...
NDIS_STATUS SetMeshSRsd(IN PRTMP_ADAPTER pAd, IN RTMP_STRING *arg)
{
	...
	if (arg) {
		if (fghavebeensend != fgmeshdetect) {
			...
			if (fgmeshdetect) {
				strncpy(MeshSdFlag, "0", sizeof(MeshSdFlag)); //There is no unknown apcli
				strncpy(SCSEnable, "1", sizeof(SCSEnable));  //scs on
			} else {
				strncpy(MeshSdFlag, "1", sizeof(MeshSdFlag)); //There is a unknown apcli
				strncpy(SCSEnable, "0", sizeof(SCSEnable));  //scs off
