...
BOOLEAN CmmIsFirstBss(struct _RTMP_ADAPTER *pAd, struct wifi_dev *wdev);
	...
	struct wifi_dev *wdev)
{
	...
	if ((pAd->CommonCfg.bIEEE80211H == 1) &&
		...
		if (DfsIsOutBandAvailable(pAd, wdev)) {
			...
			|| IS_ADJ_BW_ZERO_WAIT_CAC_DONE(pAd->CommonCfg.DfsParameter.BW160ZeroWaitState))) {		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_WARN,
